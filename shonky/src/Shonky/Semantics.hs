...
ppVal v@(VA "cons" :&&
         (VX [_] :&& _))  = dquotes (ppStringVal v)
ppVal (VA "cons"   :&&
         (v :&& w))       = brackets $ ppVal v <> ppListVal w
ppVal (VA "nil"    :&& _) = brackets empty
ppVal (VA k        :&& v) = text k <> ppConsArgs v
ppVal (VX [c])            = text $ "'" ++ [c] ++ "'"
ppVal (VF g hss es)       = text "[anon. fct.: " <+>
                      align (vcat [
                               text "hss =" <+>
                                 (text . show) hss,
                               text "es =" <+>
                                 align (vcat $ map ppClause es)] <+>
                                 text "]")
...
ppComp (Call c n vs a) = text "Call" <+> text c <> text "." <>
                         integer n <+> sep (map ppVal vs) <$$> ppAgenda a
